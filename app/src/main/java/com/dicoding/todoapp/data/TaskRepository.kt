...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        val q = FilterUtils.getFilteredQuery(filter)
        val conf = PagedList.Config.Builder().setPageSize(PAGE_SIZE)
            .setEnablePlaceholders(PLACEHOLDERS).build()
        val task = tasksDao.getTasks(q)
        return LivePagedListBuilder(task, conf).build()
    }
